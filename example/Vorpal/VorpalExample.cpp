...
int main() {
    ...
    //Register a user (optional, only needed if the user doesn't exist yet...)
    ...
    //Redeem a license for an application
    v.RedeemLicense("vorpal-TESTLICENSE", [](Vorpal* vorpal, Protected_LoginApplication* loginApp) {
        auto result = vorpal->GetReadOnly<bool>(&loginApp->Result);
        if (result.has_value()) {
